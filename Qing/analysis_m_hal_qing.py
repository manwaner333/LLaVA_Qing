...
if __name__ == "__main__":
    ...
    path = f"result/m_hal/answer_synthetic_val_data_from_M_HalDetect_llava_15_7b.bin"
    ...
    for idx, response in responses.items():
        ...
        valid_sentences_len = 0
        for ele in labels:
            if ele in ['ACCURATE', 'INACCURATE']:
                valid_sentences_len += 1

        average_logprob_sent_level = []  # [None for _ in range(sentences_len)]
        lowest_logprob_sent_level = []
        average_entropy_sent_level = []
        highest_entropy_sent_level = []

        label_True_sent_level = []
        label_False_sent_level = []
